...
import type { CurrentAuthData } from "./types";
...
export async function loadActorAccessFromAuth(
  auth: CurrentAuthData | null | undefined,
  tenantSlugFromHeader?: string,
): Promise<ActorAccess> {
  ...
  if (roles.length === 0) {
    throw APIError.permissionDenied("Anda tidak memiliki akses ke tenant ini");
  }
